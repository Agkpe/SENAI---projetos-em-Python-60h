import sqlite3 # banco de dados
import tkinter as tk # interface 
from tkinter import messagebox # caixas de mensagens
from tkinter import ttk # interface grafica tb

def conectar():
    return sqlite3.connect('teste.db')

def criar_tabela():
    conn = conectar()
    c= conn.cursor()
    c.execute('''
        CREATE TABLE IF NOT EXISTS usuarios(
        id INTEGER NOT NULL,
        nome TEXT NOT NULL,
        email TEXT NOT NULL, 
        telefone TEXT NOT NULL,
        interesse TEXT NOT NULL,
        status TEXT NOT NULL
        )       
    ''')
    conn.commit()
    conn.close()

# CREATE
def inserir_usuario():
    nome = entry_nome.get()
    email = entry_email.get()
    cpf =  entry_cpf.get()
    telefone = entry_telefone.get()
    interesse = entry_interesse.get()
    status = entry_status.get()

    if nome and email:
        conn = conectar()
        c = conn.cursor()
        c.execute('INSERT INTO usuarios(id,nome, email, telefone, interesse, status) VALUES(?,?,?,?,?,?)', (cpf,nome, email, telefone, interesse, status))
        conn.commit()
        conn.close()
        messagebox.showinfo('AVISO', 'DADOS INSERIDOS COM SUCESSO!') 
        mostrar_usuario()
    else:
        messagebox.showerror('ERRO', 'ALGO DEU ERRADO!') 

# READ
def mostrar_usuario():
    for row in tree.get_children():   
        tree.delete(row)
    conn = conectar()
    c = conn.cursor()    
    c.execute('SELECT * FROM usuarios')
    usuarios = c.fetchall()
    for usuario in usuarios:
        tree.insert("", "end", values=(usuario[0], usuario[1],usuario[2], usuario[3], usuario[4], usuario[5]))
    conn.close()  

# DELETE
def delete_usuario():
    dado_del = tree.selection()
    if dado_del:
       user_id = tree.item(dado_del)['values'][0]
       conn = conectar()
       c = conn.cursor()    
       c.execute('DELETE FROM usuarios WHERE id = ? ',(user_id,))
       conn.commit()
       conn.close()
       messagebox.showinfo('', 'DADO DELETADO')
       mostrar_usuario()

    else:
       messagebox.showerror('', 'OCORREU UM ERRO')  

# UPDATE 
       
def editar():
     selecao = tree.selection()
     if selecao:
         user_id = tree.item(selecao)['values'][0]
         novo_nome = entry_nome.get()
         novo_email = entry_email.get()
         novo_telefone = entry_telefone.get()
         novo_interesse = entry_interesse.get()
         novo_status = entry_status.get()

         if novo_nome and novo_email and novo_telefone and novo_interesse and novo_status:
            conn = conectar()
            c = conn.cursor()    
            c.execute('UPDATE usuarios SET nome = ? , email = ?, telefone = ?, interesse = ?, status = ? WHERE id = ? ',(novo_nome,novo_email, novo_telefone, novo_interesse, novo_status, user_id))
            conn.commit()
            conn.close()  
            messagebox.showinfo('', 'DADOS ATUALIZADOS')
            mostrar_usuario()

         else:
             messagebox.showwarning('', 'PREENCHA TODOS OS CAMPOS')

     else:
            messagebox.showerror('','ALGO DEU ERRADO!')


janela = tk.Tk()
janela.title('CADASTRO E-COMMERCE')


label_nome = tk.Label(janela, text='Nome:')
label_nome.grid(row=0, column=0, padx=10, pady=10)

entry_nome = tk.Entry(janela)
entry_nome.grid(row=0, column=1, padx=10, pady=10)

label_email = tk.Label(janela, text = 'E-mail:')
label_email.grid(row=1, column=0, padx=10, pady=10)

entry_email = tk.Entry(janela, text = 'E-mail:')
entry_email.grid(row=1, column=1, padx=10, pady=10)


label_CPF = tk.Label(janela, text = 'CPF:')
label_CPF.grid(row=2, column=0, padx=10, pady=10)

entry_cpf = tk.Entry(janela, text = 'CPF:')
entry_cpf.grid(row=2, column=1, padx=10, pady=10)

label_telefone= tk.Label(janela, text='Telefone:')
label_telefone.grid(row=3, column=0, padx=10, pady=10)

entry_telefone = tk.Entry(janela, text = 'Telefone:')
entry_telefone.grid(row=3, column=1, padx=10, pady=10)

label_interesse= tk.Label(janela, text='Interesse:')
label_interesse.grid(row=4, column=0, padx=10, pady=10)

entry_interesse = tk.Entry(janela, text = 'Interesse:')
entry_interesse.grid(row=4, column=1, padx=10, pady=10)


categorias = ("Pendente", "Em Andamento", "Concluído")


label_status= tk.Label(janela, text='Status:')
label_status.grid(row=5, column=0, padx=10, pady=10)

entry_status = ttk.Combobox(janela, values=categorias, state='readonly' )
entry_status.grid(row=5, column=1, padx=10, pady=10)



btn_salvar = tk.Button(janela, text='Salvar', command=inserir_usuario)
btn_salvar.grid(row=6, column=0, padx=10, pady=10)

btn_deletar = tk.Button(janela, text='deletar', command=delete_usuario )
btn_deletar.grid(row=7, column=0, padx=10, pady=10)

btn_atualizar = tk.Button(janela, text='atualizar', command=editar)
btn_atualizar.grid(row=8, column=0, padx=10, pady=10)



columns = ('ID', 'NOME', 'E-MAIL', 'TELEFONE', 'INTERESSE', 'STATUS')
tree = ttk.Treeview(janela, columns=columns, show='headings')
tree.grid(row=9,column=0,columnspan=2,padx=10, pady=10)


for col in columns:
    tree.heading(col, text=col)

criar_tabela()
mostrar_usuario()


janela.mainloop()





  
